...
import { useTransactions, type TransactionFilters as TransactionFiltersValue } from "@/hooks/useTransactions";
...
export default function TransactionsPage() {
  ...
  const currencies = useMemo(() => {
    const uniqueCurrencies = new Set(transactions.map((transaction) => transaction.currency));
  ...
  return (
    <div className="space-y-5">
      <TransactionFilters value={filters} currencies={currencies} onChange={setFilters} onExport={exportCsv} />
      <Card>
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {transactions.map((transaction) => (
                <TransactionRow key={transaction.id} transaction={transaction} onSelect={setSelectedTransaction} />
              ))}
            </TableBody>
          </Table>
      ...
      <div className="flex justify-center">
        <Button
          ...
          disabled={!transactionsQuery.hasNextPage || transactionsQuery.isFetchingNextPage}
        >
          {transactionsQuery.isFetchingNextPage ? <Loader2 className="size-4 animate-spin" /> : null}
